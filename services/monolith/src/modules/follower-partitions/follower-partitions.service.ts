...
class FollowerPartitionsService {
	...
	async getOrAssignPartition(followerId: string): Promise<number> {
		...
		const unassignedIndex = await this.repository.findUnassignedPartitionIndex(
			kafkaAdmin.getPartitionCount(KafkaTopics.PostFanOutV1),
		);
		...
		const newPartitionIndex = kafkaAdmin.getPartitionCount(KafkaTopics.PostFanOutV1);
		const newTotalCount = kafkaAdmin.getPartitionCount(KafkaTopics.PostFanOutV1) + 1;

		try {
			await kafkaAdmin.createPartitions(KafkaTopics.PostFanOutV1, newTotalCount);
